...
export async function getGateStatus(userId, userEmail = null) {
  ...
  if (!hasPaid && userEmail && process.env.STRIPE_SECRET_KEY) {
    try {
      ...
      // Search for successful checkout sessions by customer email (with pagination)
      let userSession = null;
      let hasMore = true;
      let startingAfter = undefined;

      while (hasMore && !userSession) {
...
export function determineNextRoute(gate, overrideNext) {
  ...
  // If safe override specified and user has access, use it
