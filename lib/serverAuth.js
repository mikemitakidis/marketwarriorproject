...
export async function getGateStatus(userId) {
...
export function determineNextRoute(gate, overrideNext) {
  // Check if Stripe is configured - if not, bypass payment requirement
  ...
  if (overrideNext && (gate.hasPaid || bypassPayment) && gate.welcomeCompleted) {
    return overrideNext;
  }

  // If payment is bypassed (no Stripe config), skip the /pay step
  if (bypassPayment) {
    if (!gate.welcomeCompleted) return '/welcome';
    return '/dashboard';
  }

  // Otherwise follow the gate logic
