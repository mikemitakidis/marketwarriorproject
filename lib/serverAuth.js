...
export function determineNextRoute(gate, overrideNext) {
  ...
  // FREE TRADING JOURNAL: Allow access for ALL authenticated users (no payment required)
  if (safeNext && safeNext.startsWith('/trading-journal')) {
    return safeNext;
  }
  ...
  // FREE USERS: Send to free trading journal (not the paid course)
  if (!gate.hasPaid) {
