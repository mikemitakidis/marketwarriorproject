...
export async function getGateStatus(userId, userEmail = null) {
  ...
  // Fetch access duration from settings (defaults to 120 days if not set)
  ...
  try {
    ...
    if (durationSetting?.value) {
      ...
      if (!isNaN(parsed) && parsed > 0) {
        ...
        logger.log('[GATE] Using dynamic access duration:', accessDuration, 'days');
      }
    }
  } catch (e) {
    logger.log('[GATE] Could not fetch access_duration, defaulting to 120 days');
  }

  // Get user profile (including access expiration fields)
  // NOTE: unlimited_access column might not exist if migration hasn't been run yet
  const { data: profile, error: profileError} = await admin
    .from('user_profiles')
    .select('has_paid, paid_at, full_name')
    .eq('id', userId)
    .maybeSingle();
  ...
  // Check if access has expired (dynamic duration from settings)
  let accessExpired = false;
  if (hasPaid && profile?.paid_at && !unlimited_access) {
