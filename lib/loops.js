 ...
 */
// Simple in-memory deduplication to prevent sending the same event twice
// (e.g., when Stripe webhooks retry and fire the same Loops event)
...
export async function sendLoopsEvent(email, eventName, contactProperties = {}) {
  ...
  // Deduplication: skip if same email+event was sent in last 5 minutes
  const dedupKey = `${email}:${eventName}`;
  const lastSent = _recentEvents.get(dedupKey);
  if (lastSent && Date.now() - lastSent < DEDUP_WINDOW_MS) {
    logger.log(`[LOOPS] Skipping duplicate event "${eventName}" for ${email} (sent ${Math.round((Date.now() - lastSent) / 1000)}s ago)`);
