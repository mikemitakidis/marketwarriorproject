...
export async function generateCertificatePDF(fullName, certificateId, completionDate) {
  ...
  //    Auto-scale: keep name within 500px safe zone (well inside borders)
  const maxNameWidth = 500;
  let nameSize = 44.8;
  let nameWidth = font.widthOfTextAtSize(fullName, nameSize);
  while (nameWidth > maxNameWidth && nameSize > 18) {
    nameSize -= 0.5;
    nameWidth = font.widthOfTextAtSize(fullName, nameSize);
  }
