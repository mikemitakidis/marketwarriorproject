...
export function checkSpam(text) {
  ...
  if (linkCount > AUTO_FLAG_LINK_COUNT) {
    return { ok: false, reason: 'Too many links. Maximum 3 links allowed per post.', shouldAutoFlag: true };
  }

  if (linkCount > MAX_LINKS) {
    return { ok: false, reason: 'Too many links. Maximum 3 links allowed per post.', shouldAutoFlag: false };
  }
