...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      // Collect all trade IDs from all playbooks and batch-fetch (avoids N+1 queries)
      const allTradeIds = new Set();
      (playbooks || []).forEach(p => {
        (p.example_trade_ids || []).forEach(id => allTradeIds.add(id));
      });
      ...
      if (allTradeIds.size > 0) {
        const { data: allTrades } = await supabase
          .from('journal_trades')
          .select('id, pnl_amount, r_multiple')
          .in('id', [...allTradeIds])
          .eq('status', 'closed');

        (allTrades || []).forEach(t => { tradeMap[t.id] = t; });
      }

      // Calculate stats using pre-fetched trade data
      const playbooksWithStats = (playbooks || []).map((playbook) => {
        if (playbook.example_trade_ids && playbook.example_trade_ids.length > 0) {
          const trades = playbook.example_trade_ids
            .map(id => tradeMap[id])
            .filter(Boolean);
          ...
          if (trades.length > 0) {
            ...
            score += Math.min(30, (Math.min(profitFactor, 3) / 3) * 30);
            score += Math.min(30, ((avgR + 1) / 2) * 30);
