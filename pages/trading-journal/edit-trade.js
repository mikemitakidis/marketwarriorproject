...
export async function getServerSideProps({ req, res, query }) {
  try {
    ...
    const { id } = query;
    if (!id) {
      return { redirect: { destination: '/trading-journal/trades', permanent: false } };
    }

    const supabase = getServiceSupabase();

    const [settingsResult, tradeResult] = await Promise.all([
      supabase
    ...
    const tradeTags = trade.journal_trade_tags?.map(tt => tt.journal_tags).filter(Boolean) || [];
    ...
    return {
      props: {
        ...
        trade: {
          ...
          execution_score: trade.execution_score != null ? String(trade.execution_score) : '',
          confidence_rating: trade.confidence_rating != null ? String(trade.confidence_rating) : '',
          ...
          swap_fees: trade.swap_fees ?? 0,
          status: trade.status || 'open',
        },
        tradeTags: tradeTags.map(t => t.id),
      },
    };
  } catch (err) {
    return { redirect: { destination: '/trading-journal/login', permanent: false } };
...
export default function EditTradePage({ user, settings, trade, tradeTags }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [tags, setTags] = useState({ setup: [], mistake: [], emotion: [], market_condition: [] });
  const [selectedTags, setSelectedTags] = useState(tradeTags || []);
  ...
  return (
    <JournalLayout user={user} title="Edit Trade" settings={settings}>
      <style jsx global>{`
      ...
      <div className="dashboard-header-tabs">
        ...
        <Link href="/trading-journal/trades">Trade Log</Link>
          ...
          <div className="form-main">
            ...
            <div className="form-section">
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  <label className="form-label">Swap Fees</label>
                  <input
                    type="number"
                    name="swap_fees"
                    className="form-input"
