...
* **Stripe Payments** – `/api/checkout/stripe` creates a checkout
  ...
  for completed sessions to grant access.  The active price ID is
  stored in the `app_settings` table and can be updated through the
  admin panel.  Promotion codes are supported for testing.  Price IDs
  are validated against environment variables as a fallback.
...
* **Admin Panel** – The admin page scaffolding includes authentication,
  user management and a settings section.  An endpoint
  `/api/admin/settings/stripe-price` allows admins to update the
  Stripe price ID without redeploying the site.  Extend the panel with
  full CRUD operations for course content, announcements, user
